...
struct HeartRateTimelineChartView: View {
    ...
    private var effectiveEndTime: Date { record.endTime ?? Date() }

    private func fetchSamples() {
        let start = record.startTime.addingTimeInterval(-3600)
        let end = effectiveEndTime.addingTimeInterval(3600)
    ...
    private var peakBPM: Int { samples.filter { $0.timestamp >= record.startTime && $0.timestamp <= effectiveEndTime }.map(\.bpm).max() ?? 0 }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Chart {
                            ...
                            RectangleMark(
                                ...
                                xEnd:   .value("End",   relativeMinutes(effectiveEndTime)),
